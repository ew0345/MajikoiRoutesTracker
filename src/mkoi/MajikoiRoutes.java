...
public class MajikoiRoutes extends JFrame {
	...
	public MajikoiRoutes() throws IOException {
		...
		mDeleteData.setBounds(22, 395, 367, 116);
		about.add(mDeleteData);

		JButton mResetBoxes = new JButton("Reset Boxes");
		mResetBoxes.setFont(new Font("Tahoma", Font.PLAIN, 30));
		mResetBoxes.setBounds(388, 395, 367, 116);
		about.add(mResetBoxes);


		// Persistence code for checkboxes

		List<JCheckBox> m1Boxes = new ArrayList<JCheckBox>();
		List<JCheckBox> mSBoxes = new ArrayList<JCheckBox>();
		List<JCheckBox> mA1Boxes = new ArrayList<JCheckBox>();
		List<JCheckBox> mA2Boxes = new ArrayList<JCheckBox>();
		List<JCheckBox> mA3Boxes = new ArrayList<JCheckBox>();
		List<JCheckBox> mA4Boxes = new ArrayList<JCheckBox>();
		List<JCheckBox> mA5Boxes = new ArrayList<JCheckBox>();

		for (Component comp : majikoi1.getComponents() ) {
			if (comp instanceof JCheckBox) m1Boxes.add((JCheckBox) comp);
		}

		for (Component comp : majikoiS.getComponents() ) {
			if (comp instanceof JCheckBox) mSBoxes.add((JCheckBox) comp);
		}

		for (Component comp : majikoiA1.getComponents() ) {
			if (comp instanceof JCheckBox) mA1Boxes.add((JCheckBox) comp);
		}

		for (Component comp : majikoiA2.getComponents() ) {
			if (comp instanceof JCheckBox) mA2Boxes.add((JCheckBox) comp);
		}

		for (Component comp : majikoiA3.getComponents() ) {
			if (comp instanceof JCheckBox) mA3Boxes.add((JCheckBox) comp);
		}

		for (Component comp : majikoiA4.getComponents() ) {
			if (comp instanceof JCheckBox) mA4Boxes.add((JCheckBox) comp);
		}

		for (Component comp : majikoiA5.getComponents() ) {
			if (comp instanceof JCheckBox) mA5Boxes.add((JCheckBox) comp);
		}
		...
		mDeleteData.addActionListener((ActionEvent e) -> {
			...
			if (q == JOptionPane.YES_OPTION) {
				...
				for (JCheckBox box : m1Boxes) {
					box.setSelected(false);
				}

				for (JCheckBox box : mSBoxes) {
					box.setSelected(false);
				}

				for (JCheckBox box : mA1Boxes) {
					box.setSelected(false);
				}

				for (JCheckBox box : mA2Boxes) {
					box.setSelected(false);
				}

				for (JCheckBox box : mA3Boxes) {
					box.setSelected(false);
				}

				for (JCheckBox box : mA4Boxes) {
					box.setSelected(false);
				}

				for (JCheckBox box : mA5Boxes) {
					box.setSelected(false);
				}
			}
		});

		mResetBoxes.addActionListener((ActionEvent e) -> {
			int q = JOptionPane.showConfirmDialog(this, "Reset Checkboxes?\nThis will reset the state of all checkboxes.", "Reset Checkboxes",
